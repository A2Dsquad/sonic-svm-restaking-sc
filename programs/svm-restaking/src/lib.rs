...
declare_id!("E5BhQ9db995p1X5L5rHfmueZB2CjzWYPRkRrhau8ab3G");

#[program]
pub mod svm_restaking {
    use super::*;

    /// Claims all pending rewards accumulated on  & $indexMESH.
    ///
    /// # Arguments
    ///
    ...
    /// * `mesh_mint` - Pubkey of the  SPL token.
    ...
    /// Stakes user  or $indexMESH tokens, along with pending reward calculation and distribution.
    ///
    /// # Arguments
    ///
    /// * `ctx` - The accounts required by the instruction.
    ...
    /// Unstakes user  or $indexMESH tokens, along with pending reward calculation and distribution.
    ///
    /// # Arguments
    ///
    /// * `ctx` - The accounts required by the instruction.
