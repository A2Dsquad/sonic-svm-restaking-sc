...
pub const DEPLOYER: Pubkey = pubkey!("4yLVEowhDEy4Xrs9Quaqka5Yq96iZSnmPDfENJrpFz3j");
...
pub struct GlobalState {
    ...
    /// Global state for maintaining reward calculation per unit of .
    ...
    /// Total amount of  staked by users.
    ...
    /// Note: the weightage is in (/$indexMESH), e.g., 0.5 means 0.5  is equivalent to 1 $indexMESH.
    pub weightage: u64,
...
impl GlobalState {
    ...
    /// Calculate the newly accumulated reward on  & $indexMESH and sum it with previously accumulated reward.
    ...
    /// Update the global state of reward accumulated per unit of  & $indexMESH.
...
pub struct UserInfo {
    /// Pubkey of user/
    pub user: Pubkey,
    /// User state for maintaining reward calculation per unit of .
...
/// PDA to hold the ownership of reward SOLs, staked , and $indexMESH.
#[account]
